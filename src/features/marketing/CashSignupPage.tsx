...
import { requestCashSignup } from './api'
...
const schema = z.object({
  ...
  owner_full_name: z.string().trim().min(2, 'Ingresa tu nombre'),
  owner_email: z.email('Ingresa un correo válido'),
  owner_phone: z.string().trim().min(10, 'Ingresa un teléfono a 10 dígitos'),
  plan: z.enum(['monthly', 'annual']),
})

type FormValues = z.infer<typeof schema>

export function CashSignupPage() {
  ...
  async function onSubmit(values: FormValues) {
    setServerError(null)
    try {
      await requestCashSignup(values)
      setSent(true)
    } catch (err) {
      setServerError(err instanceof Error ? err.message : 'No se pudo enviar tu solicitud')
    }
  }
      ...
      >
        {sent ? (
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <h1 className="text-carbon-900 dark:text-paper text-xl font-bold">¡Listo, ya casi!</h1>
            <p className="text-carbon-500 dark:text-carbon-400 text-sm">
              Recibimos tus datos. Te contactaremos por WhatsApp o llamada al teléfono que nos diste
              para coordinar el pago en efectivo — en cuanto lo confirmemos, tu tienda queda activa.
            </p>
            <Link
              to="/"
          ...
          <>
            <div className="mb-6 flex flex-col items-center gap-2 text-center">
              ...
              <h1 className="text-carbon-900 dark:text-paper text-xl font-bold">
                Regístrate para pagar en efectivo
              </h1>
              <p className="text-carbon-500 dark:text-carbon-400 text-sm">
                Deja tus datos y te contactamos para coordinar el pago — sin tarjeta, sin
                complicaciones.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
              ...
              <TextField
                label="Tu teléfono (WhatsApp de preferencia)"
                type="tel"
            ...
            <p className="text-carbon-400 mt-4 text-center text-xs">
              ¿Prefieres pagar con tarjeta y activar tu tienda al instante?{' '}
              <Link
                to="/registro"
